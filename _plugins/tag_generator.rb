module Jekyll
  ...
  class TagPage < BaseTagPage
    def initialize(site, base, dir, tag)
      super(site, base, dir)
      self.data['tag'] = tag
      tag_data = site.config['tag']['page']
      title = tag_data['title']
      header = tag_data['header'];
      description = tag_data['description']
      metadesc = tag_data['metadesc']
      tag_name = site.data['tags'][tag]['name']
      self.data['title'] = sprintf( title, tag_name )
      self.data['header'] = sprintf( header, tag_name )
      self.data['description'] = sprintf( description, tag_name )
      self.data['metadesc'] = sprintf( metadesc, tag_name )
    end
  ...
  class TagPageGenerator < Generator
    ...
    def generate(site)
      if site.layouts.has_key? 'tag_page'
        dir = site.config['tag']['page']['dir'] || 'tag'
  ...
  class TagIndexGenerator < Generator
    ...
    def generate(site)
      if site.layouts.has_key? 'tag_index'
        dir = site.config['tag']['index']['dir'] || 'tags'
